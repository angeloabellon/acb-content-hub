...
import { buildPublicEpisodeCandidate, diffPublicEpisode } from "@/lib/episode-promotion";
import { isEpisodePromotionStorageAvailable } from "@/lib/episode-promotion-storage";
...
export default async function EpisodeEditorialDashboard({ params }: PageProps) {
  ...
  if (!episode) notFound();
  const imports = await loadImportedEpisodeManifests();
  const summary = await getEpisodeEditorialSummary(episode, { loadImportedTranscripts, loadImportedManifests: async () => imports });
  ...
  return <main className="mx-auto max-w-6xl px-6 py-12 md:py-16">
    ...
    <nav aria-label="Módulos editoriales" className="mt-8 flex flex-wrap gap-2 border-y border-red-900/40 py-4 text-sm"><a href="#resumen" className="rounded-lg px-3 py-2 text-white/75 hover:bg-white/10">Resumen</a><a href="#transcripcion" className="rounded-lg px-3 py-2 text-white/75 hover:bg-white/10">Transcripción</a><a href="#capitulos" className="rounded-lg px-3 py-2 text-white/75 hover:bg-white/10">Capítulos</a><a href="#publicacion" className="rounded-lg px-3 py-2 text-white/75 hover:bg-white/10">Publicación</a><a href="#promocion" className="rounded-lg px-3 py-2 text-white/75 hover:bg-white/10">Promoción</a><span className="cursor-not-allowed rounded-lg px-3 py-2 text-white/30">Piezas derivadas · Próximamente</span><a href="#diagnostico" className="rounded-lg px-3 py-2 text-white/75 hover:bg-white/10">Diagnóstico</a></nav>
